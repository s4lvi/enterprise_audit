...
export default async function ChapterChecklistPage() {
    ...
    <main className="mx-auto mt-8 w-full max-w-4xl px-4 sm:px-6 lg:px-8">
      <header className="mb-6 flex flex-wrap items-baseline justify-between gap-3">
        <div>
          ...
          <h1 className="text-3xl">Chapter checklist items</h1>
          <p className="mt-2 text-sm tracking-wide text-white/60">
            Configurable checks shown on every chapter edit form.
          </p>
        </div>
        <Button asChild size="sm">
          <Link href="/admin/chapter-checklist/new">New item</Link>
        </Button>
      </header>
            ...
            {(items ?? []).length === 0 ? (
              <tr>
                <td colSpan={4} className="p-6 text-center text-white/50">
                  No checklist items yet. Add one to populate the chapter form.
                </td>
              </tr>
            ) : (
              (items ?? []).map((it) => (
                <tr key={it.id} className="border-t border-white/10 hover:bg-white/5">
                  ...
                  <td className="p-3 text-right">
                    <Link
                      href={`/admin/chapter-checklist/${it.id}`}
